...
size_t crawlpp::StringTools::strlen(const char *str) {
  if (sizeof(char) != 1)
...
char *crawlpp::StringTools::strrnchr(char *ptr, size_t len, char ch) {
  static_assert(
      sizeof(char) == 1,
  ...
  ptr_type wide_checker = ((ptr_type)ch << 0) | ((ptr_type)ch << 8) |
                          ((ptr_type)ch << 16) | ((ptr_type)ch << 24);
  if (sizeof(ptr_type) == sizeof(uint64_t))
    wide_checker |= ((ptr_type)ch << 32) | ((ptr_type)ch << 40) |
                    ((ptr_type)ch << 48) | ((ptr_type)ch << 56);
...
char *crawlpp::StringTools::strrnstrn(char *ptr, size_t ptrlen,
...
void crawlpp::StringTools::strnset(char *ptr, char ch, size_t len) {
  ...
  if (len > 0) {
    ptr_type put = ((ptr_type)ch << 0) | ((ptr_type)ch << 8) |
                   ((ptr_type)ch << 16) | ((ptr_type)ch << 24);
    if (sizeof(ptr_type) == sizeof(uint64_t))
      put |= ((ptr_type)ch << 32) | ((ptr_type)ch << 40) |
             ((ptr_type)ch << 48) | ((ptr_type)ch << 56);
...
size_t crawlpp::StringTools::strcountch(char *ptr, char *last, char ch) {
...
String::String(char *str, size_t len, bool built_in)
    : length(len), first(str), last(str + len - 1), tm(built_in) {}

String::String(char ch, size_t count) : length(count) {
  first = alloc(length + 1);
  last = first + length - 1;
  crawlpp::StringTools::strnset(first, ch, count);
  first[length] = 0;
}

String::String(char ch) : length(1) {
  first = alloc(2);
  last = first;
  *first = ch;
  *(last + 1) = 0;
...
String String::Concat(const String &t1, const String &t2, const String &t3) {
  ...
  memcpy(mergerString, t1.first, t1.length * sizeof(char));
  memcpy(mergerString + t1.length, t2.first, t2.length * sizeof(char));
  memcpy(mergerString + t1.length + t2.length, t3.first,
         t3.length * sizeof(char));
  mergerString[newSize] = 0;
...
String String::PadLeft(size_t len, char pad) {
  if (len > length) {
    ...
    crawlpp::StringTools::strnset(ret, pad, padlen);
...
String String::PadRight(size_t len, char pad) {
  if (len > length) {
    ...
    crawlpp::StringTools::strnset(ret + length, pad, len - length);
...
String String::PadCenter(size_t len, char pad, bool lefts) {
  if (len > length) {
    ...
    crawlpp::StringTools::strnset(ret, pad, lpadlen);
    memcpy(ret + lpadlen, first, length * sizeof(char));
    crawlpp::StringTools::strnset(ret + lpadlen + length, pad, rpadlen);
...
String String::Remove(size_t starts, size_t len) {
  ...
  memcpy(neString + starts, first + starts + len,
         (retlen - starts) * sizeof(char));
...
String String::Slicing(size_t jmp, size_t starts, size_t len, bool remain) {
...
char *String::ToAnsi() { return UnicodeToAnsi(); }
...
String &crawlpp::String::operator=(String &&refer) {
  this->Swap(refer);
  refer.tm = this->tm;
  this->tm = false;
  return *this;
...
String::ArrayType String::SplitHelper(const char *src, size_t srclen,
                                      size_t max) {
...
String::ArrayType String::SplitSlowHelper(const char *src, size_t srclen,
                                          size_t max) {
  size_t max_dec = max;
  size_t count = 0;
  size_t i = 0;
...
String String::SplitPositionHelper(const char *src, size_t srclen, size_t pos) {
  char *ptr = first, *tptr;
...
String::ArrayType String::SplitReverseHelper(const char *src, size_t srclen,
                                             size_t max) {
  size_t alloclen = max <= length ? max : length;
  char **position = new char *[alloclen];
  size_t *poslen = new size_t[alloclen];
  ...
  for (; (tptr = StringTools::strrnstrn(first, nowlen, src, srclen)) && max;
       max--, count++) {
...
String String::BetweenHelper(const char *left, size_t llen, const char *right,
...
String::ArrayType String::BetweensHelper(const char *left, size_t llen,
...
String String::Between(char left, char right, size_t starts) {
  ...
  return (lefts != error) && (rights != error) ? Slice(lefts, rights - 1)
                                               : String();
}

String::ArrayType String::Betweens(char left, char right, size_t starts) {
  char **position = new char *[length];
  size_t *poslen = new size_t[length];
  size_t count = 0;
...
String String::InsertLeftHelper(size_t separation, const char *str,
                                size_t strlen) {
...
String String::InsertRightHelper(size_t separation, const char *str,
                                 size_t strlen) {
  size_t sizeof_diff = (length - 1) / separation;
  size_t len = sizeof_diff * strlen + length;
  size_t dstrlen = strlen * sizeof(char);
...
String String::ReplaceHelper(const char *src, const char *dest, size_t srclen,
                             size_t destlen, size_t max) {
  size_t alloclen = max <= length ? max : length;
  char **position = new char *[alloclen];
  size_t count = 0;
  size_t sourceLength;
  ...
  for (; index < count;
       index++, iter += srclen + tlen, mergerPointer += destlen) {
...
String String::ReplaceSlowHelper(const char *src, const char *dest,
  ...
  for (ptr = first; (tptr = strstr(ptr, src)) && max; max--,
      iter += srclen + tlen, ptr = tptr + srclen, mergerPointer += destlen) {
...
String String::TrimHelper(const char *src, size_t srclen, size_t max) {
  ...
  for (; index < count; index++, iter += srclen + tlen, mergerPointer += tlen) {
    tlen = (size_t)((const char *)position[index] - iter);
...
String String::InsertHelper(size_t starts, const char *str, size_t len) {
  ...
  memcpy(neString + starts + len, first + starts,
         (length - starts) * sizeof(char));
...
String::ArrayType String::LineSplitHelper(size_t len, const char *front,
                                          size_t front_len, const char *end,
                                          size_t end_len) {
  ...
  if (remainLen != 0) {
    if (end_len) {
      crawlpp::StringTools::strnset(n[countLineLen - 1]->first + front_len +
                                         remainLen,
                                     L' ', len - remainLen);
    } else {
