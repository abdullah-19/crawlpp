...
public:
  ArrayBase() : m_size(0), m_pointer(nullptr), m_constptr(nullptr) {}

  ArrayBase(size_t size, type *pointer)
      : m_size(size), m_pointer(pointer), m_constptr(pointer) {}
  ...
  template <typename tt> void _class_delete(tt &, tt *&) {}
  template <typename tt> void _class_delete(tt *&, tt *const *&arr) {
...
template <typename type> class ReadOnlyArray : public ArrayBase<type> {
...
public:
  ...
  bool operator!=(const this_type &refer) const { return (!(*this == refer)); }
  bool operator<(const this_type &refer) const {
    return (this->m_pointer < refer.m_pointer);
  }
  ...
  template <typename func> void Each(func function) {
...
public:
  ...
  static char *strrnstrn(char *ptr, size_t ptrlen, const char *dest,
                         size_t destlen);
...
public:
  ...
  String(float);
  String(double);
  String(String &&ws) : first(ws.first), last(ws.last), length(ws.length) {
    ws.tm = true;
  }
...
private:
  ...
  ArrayType BetweensHelper(const char *left, size_t llen, const char *right,
                           size_t rlen, size_t starts);
  ...
  String InsertLeftHelper(size_t separation, const char *str, size_t strlen);
  String InsertRightHelper(size_t separation, const char *str, size_t strlen);

  String ReplaceHelper(const char *src, const char *dest, size_t srclen,
                       size_t destlen, size_t max);
  String ReplaceSlowHelper(const char *src, const char *dest, size_t srclen,
                           size_t destlen, size_t max);
  ...
  ArrayType LineSplitHelper(size_t len, const char *front, size_t front_len,
...
public:
  ...
  String Between(const String &left, const String &right, size_t starts = 0) {
    return BetweenHelper(left.first, left.length, right.first, right.length,
                         starts);
  }
  ArrayType Betweens(const char *left, const char *right, size_t starts = 0) {
    return BetweensHelper(left, StringTools::strlen(left), right,
                          StringTools::strlen(right), starts);
  }
  ArrayType Betweens(const String &left, const String &right,
                     size_t starts = 0) {
    return BetweensHelper(left.first, left.length, right.first, right.length,
                          starts);
  }
  ...
  inline bool StartsWith(const char ch) const { return *first == ch; }
  ...
  inline bool EndsWith(const char ch) const { return *last == ch; }
  ...
  String Replace(const char *src, const char *dest, size_t max = SIZE_MAX) {
    return ReplaceHelper(src, dest, StringTools::strlen(src),
                         StringTools::strlen(dest), max);
  }
  ...
  String ReplaceSlow(const char *src, const char *dest, size_t max = SIZE_MAX) {
    return ReplaceSlowHelper(src, dest, StringTools::strlen(src),
                             StringTools::strlen(dest), max);
  }
  ...
  ArrayType LineSplit(size_t len, const char *front) {
    return LineSplitHelper(len, front, StringTools::strlen(front), nullptr, 0);
  }
  ...
  ArrayType LineSplit(size_t len, const char *front, const char *end) {
    return LineSplitHelper(len, front, StringTools::strlen(front), end,
                           StringTools::strlen(end));
  }
  ...
  std::string ToStr() const { return std::string(first); }
...
inline String operator"" _s(unsigned long long i) { return String{i}; }
