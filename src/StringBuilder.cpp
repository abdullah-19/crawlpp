...
void StringBuilder::Append(const String &refer) {
  if (!refer.Empty()) {
    if (capacity > refer.Length() + m_last->m_length) {
      ...
      memcpy(m_last->m_ptr + m_last->m_length, refer.Reference(),
             refer.Length() * sizeof(char));
...
String StringBuilder::ToString() {
  ...
  StringBuilderNode *iter = m_head;
  for (; iter != nullptr; iter = iter->m_next) {
    memcpy(neString + iter->m_offset, iter->m_ptr,
           iter->m_length * sizeof(char));
  }
  neString[len] = 0;
...
bool StringBuilder::MatchContinue(const String &str, size_t start_offset,
  ...
  if (str.Length() - start_offset < start_node->m_length - node_offset) {
    if (String(start_node->m_ptr, start_node->m_length)
            .FindFirst(str.Reference() + start_offset, node_offset) !=
        String::error)
      return true;
    return false;
  }
...
StringBuilder::StringBuilderNode *
StringBuilder::ReplaceInternal(const String &src, const String &tar,
                               StringBuilderNode *start_node,
                               size_t node_offset) {
